...
const GetImages = (folderPath) => {
  ...
  // 通过axios的get方法和github api获取图片列表
  axios.get({
...
const DeleteImage = (item) => {
  emit('SetLoading', true)
  // 通过axios的delete方法和github api删除图片
  axios.delete({
...
// 删除文件夹
const DeleteForder = () => {
  if (files.value.filter((file) => file.name !== 'init').length === 0) {
    loading.value = true
    // 通过axios的delete方法和github api删除文件夹
    axios.delete({
...
const FormatWImageInfo = (image) => {
  return `
    ...
    <div class="item"><span>大小</span><div>${GetFileSize(image.size)}</div></div>
    ...
    <div class="item"><span>download_url</span><div>${image.download_url}</div> </div>`
}
